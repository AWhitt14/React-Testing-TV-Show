...
test('tests app component for functionality', async () => {
    ...
    const {getByRole, getAllByTestId, findByText,findByPlaceholderText} = render(<App />);

    await findByText(/Fetching data.../);
    const posterImg = getByRole('img', {className: /poster-img/i});
    
    expect(posterImg.src).toEqual('http://localhost/imgSrc');
